# Deploying the kubernetes cluster (K3S + Calico)
...
##  ⚙️ 0) Install K3s (with flannel disabled)
* Run on the **Master Node**: <br />
``` export INSTALL_K3S_VERSION=v1.33.4+k3s1 && curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--flannel-backend=none --disable-network-policy --cluster-cidr=10.42.0.0/16" sh -s - --token mamoutou@2025```
   * Get the K3S token:  ``` sudo cat /var/lib/rancher/k3s/server/node-token```

* Run on each **Worker Node**: <br />
...
```
* Wait for all calico pods to be ready: <br />
